...
* A file containing the dataflow configuration, e.g. `controller/examples/offline/flink_wordcount_topology.csv`, and
* A log file with the collected operator rates for one or more time windows (epochs), e.g. `controller/examples/offline/flink_rates.log`

Go to `controller/` and run the following command to evaluate the scaling policy for each epoch in the rate file assuming Flink as the running system:
