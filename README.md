...
* Updates its state every time a new rate file is created. The scaling manager expects one rate file per operator instance and time window (epoch) following the naming convention 'some_name-epoch_number.file_extension'. Epoch numbers can be arbitrary. The scaling manager will only consider epochs that generate exactly one rate file per operator instance.
...
The following command evaluates the scaling policy for each time window in the rate file assuming Flink as the running system:
